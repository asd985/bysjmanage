...
public interface ITaskDao {
    ...
    @MapKey("status")
    @Select("select IFNULL(status,0) as status,count(*) as count from task where `status`=0 " +
            "UNION " +
            "select IFNULL(status,1),count(*) as count from task where `status`=1 " +
            "UNION " +
            "select IFNULL(status,2),count(*) as count from task where `status`=2 ")
